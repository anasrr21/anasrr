...
async def mutejep(event):
    if event.is_private:
        ...
        if event.chat_id == 1096229060:
            return await edit_delete(event, "** دي . . لا يمڪنني كتـم اخو مطـور السـورس  ╰**")
        try:
        ...
        if user.id == 5214682327:
            return await edit_or_reply(event, "** دي . . لا يمڪنني كتـم  مطوري السـورس  ╰**")
