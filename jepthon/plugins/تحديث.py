...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**᯽︙ قام مطور السورس بتحديث سيرثون**\n᯽︙ **التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ᯽︙ تم تحديث سورس سيرثون بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ يـتـم البـحـث عـن تـحديثـات سـورس سيرثون انـتـظـر**")
    off_repo = UPSTREAM_REPO_URL
    force_update = False
    if HEROKU_API_KEY is None or HEROKU_APP_NAME is None:
        return await edit_or_reply(
            event, "`API HEROKU KEY او APP HEROKU NAME خطا تأكد من فاراتك عبر موقع هيروكو`"
        )
    try:
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس سيرثون ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ᯽︙ جار تحـديـث سـورس سيرثون انـتـظـر قـليـلا 🔨**")
